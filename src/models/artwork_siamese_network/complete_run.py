...
class CompleteRun(Run):
    ...
    def encode_raw_info(self, data_dict: dict[str, dict]):
        for ix, (k, d) in enumerate(data_dict.items()):
            target = (
                self.train_loader.dataset.source_modalities
                if ix == 0
                else self.train_loader.dataset.dest_modalities
            )
            for data_t, tensor in d.items():
                if self.train_loader.dataset.mode[data_t] == Mode.RAW.value and data_t in target:
